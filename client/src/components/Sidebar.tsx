        ...
        <div className='flex flex-col h-full'>
                    ...
                    if(isMessage){
                        ...
                        return(
                            <div key={msg.id} className={`flex items-start gap-3 ${isUser ? 'justify-end' : 'justify-start'}`} >
                                ...
                                <div className={`max-w-{80%} p-2 px-4 rounded-2xl shadow-sm text-sm mt-5 leading-relaxed ${isUser ? 'bg-linear-to-r from-indigo-500 to indigo-600 text-white rounded-tr-none' : 'bg-gray-800 text-gray-100 rounded-tl-none'}`}>
                                    {msg.content}
                                </ div>
                                {isUser && (
                ...
                {isGenerating && (
                    <div className='flex items-start gap-3 justify-start'>
                        ...
                        <div className='flex items-end gap-1.5 h-full'>
                            <div className='size-2 rounded-full bg-indigo-600 animate-bounce' style={{animationDelay:'0s'}}/>
                            <div className='size-2 rounded-full bg-indigo-600 animate-bounce' style={{animationDelay:'0.2s'}}/>
                            <div className='size-2 rounded-full bg-indigo-600 animate-bounce' style={{animationDelay:'0.4s'}}/>
                        </div>
                    </div>
                )}
                <div ref={messageRef} />
            ...
            <form onSubmit={handleRevision} className='m-3 relative'>
                <div className='flex items-center gap-2'>
                    <textarea value={input} onChange={(e) => setInput(e.target.value)} rows={4} placeholder='Describe your website or request changes...' 
                    className='flex-1 p-3 rounded-xl resize-none text-sm outline-none ring ring-gray-700 focus:ring-indigo-500 bg-gray-800 text-gray-100 placeholder-gray-400 transition-all' disabled={isGenerating} />
                    <button disabled={isGenerating || !input.trim()} className='absolute bottom-2.5 right-2.5 rounded-full bg-linear-to-r from -indigo-500 to-indigo-600 hover:from-indigo-600 hover:to-indigo-700 text-white transition-colors'>
                        {isGenerating ? (
