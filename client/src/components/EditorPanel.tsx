...
const EditorPanel = ({selectedElement, onUpdate, onClose} : EditorPanelProps) => {
    ...
    const handleStyleChange = (value:string, field:string)=>{
        const newStyles = {...values.styles, [field]:value};
        setValues({...values, styles:newStyles} );
        onUpdate({styles:{[field]:value}});
    }

  return (
    <div className='absolute top-4 right-4 w-80 bg-white rounded-lg shadow-xl border border-gray-200 p-4 z-50 animate-in fade slide-in-right-5'>
        ...
        <div className='space-y-4 text-black'>
            <div>
                ...
                <textarea value={values.text} onChange={(e)=>handleChange(e.target.value, 'text')} 
            ...
            <div>
                ...
                <input type="text" value={values.className} onChange={(e)=>handleChange(e.target.value, 'className')} 
            ...
            <div className='grid grid-cols-2 gap-3'>
                <div>
                    ...
                    <input type="text" value={values.styles.padding} onChange={(e)=>handleStyleChange(e.target.value, 'padding')} 
                    className='w-full text-sm p-2 border border-gray-400 rounded-md focus:ring-2 focus:ring-indigo-500 outline-none '/>
                </div>
                <div>
                    <label className='block text-xs font-medium text-gray-500 mb-1'>Margin</label>
                    <input type="text" value={values.styles.margin} onChange={(e)=>handleStyleChange(e.target.value, 'margin')} 
                    className='w-full text-sm p-2 border border-gray-400 rounded-md focus:ring-2 focus:ring-indigo-500 outline-none '/>
                </div>
            </div>
            <div className='grid grid-cols-2 gap-3'>
                <div>
                    ...
                    onChange={(e)=>handleStyleChange(e.target.value, 'fontSize')} 
                    className='w-full text-sm p-2 border border-gray-400 rounded-md focus:ring-2 focus:ring-indigo-500 outline-none '/>
                </div>
            </div>
            <div className='grid grid-cols-2 gap-3'>
                <div>
                    ...
                    <div className='flex items-center gap-2 border border-gray-400 rounded-md p-1'>
                        {/* Changed type="background" to type="color" */}
                        <input type="color" 
                        value={values.styles.backgroundColor==='rgba(0,0,0,0)'?'#ffffff':values.styles.backgroundColor} 
                        onChange={(e)=>handleStyleChange(e.target.value, 'backgroundColor')} 
                ...
                <div>
                    ...
                    <div className='flex items-center gap-2 border border-gray-400 rounded-md p-1'>
                        ...
                        onChange={(e)=>handleStyleChange(e.target.value, 'color')} 
                        className='w-6 h-6 cursor-pointer'/>
